...
export class CookingPuzzleScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(this.gameCenterX, height - 20, puzzleData.dishName, {
    ...
    scatterCards() {
        ...
        if (!this.processorCards.has(puzzleData.finalStep.processor)) {
            const pos = findPosition();
            const emoji = puzzleData.finalStep.processorEmoji || procEmojiMap.get(puzzleData.finalStep.processor) || '';
            const card = this.createCard(pos.x, pos.y, puzzleData.finalStep.processor, 'processor', { emoji });
