      ...
      >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 3 }}>
              ...
              <div style={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                {Array.from({ length: Math.min(survivalLives, 10) }).map((_, i) => (
                  <Heart key={i} size={9} strokeWidth={2} fill="#e53935" color="#e53935" />
                ))}
              </div>
            </div>
          )}
        ...
        >
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <div style={{ height: 5, background: '#f0e8df', borderRadius: 3, overflow: 'hidden' }}>
              <div
                style={{
                  height: '100%',
                  width: `${totalSteps > 0 ? (stepCount / totalSteps) * 100 : 0}%`,
                  background: stepCount === totalSteps ? '#4caf50' : '#ffca28',
                  borderRadius: 3,
          ...
          {puzzleData.branches.map((branch) => (
            <div key={branch.name} style={{ marginBottom: 4 }}>
              <BranchLabel label={localize(branch.name, branch.nameI18n)} />
          ...
          <div style={{ marginBottom: 4 }}>
            <BranchLabel label={t('quest.final')} />
            <StepRow
...
function BranchLabel({ label }: { label: string }) {
  return (
    ...
    >
      <div style={{ flex: 1, height: 1, background: '#ede0d4', borderRadius: 1 }} />
      {label}
      <div style={{ flex: 1, height: 1, background: '#ede0d4', borderRadius: 1 }} />
    </div>
  );
    ...
    >
      ...
      <div
        style={{
          ...
          ...(completed
            ? { background: '#4caf50' }
            : actionable
              ? { border: '2px solid #ffca28', background: '#fff8e1' }
              : { border: '1.5px solid #d0d0d0', background: 'transparent' }),
        }}
      >
        {completed && <Check size={9} strokeWidth={3.5} color="#fff" />}
        {actionable && (
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#ffca28' }} />
        )}
      </div>
      ...
      <img
        ...
        style={{
          ...
          marginRight: 5,
          marginTop: 1,
          opacity: completed ? 0.6 : actionable ? 1 : 0.3,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
          <span
            style={{
              ...
              fontSize: 10,
              fontWeight: 900,
              color: completed ? '#4caf50' : actionable ? '#3e2723' : '#b0a09a',
          ...
          {!completed && (
            <span
              style={{
                fontSize: 9,
                color: actionable ? '#a1887f' : '#c8bbb5',
                flexShrink: 0,
                display: 'flex',
                alignItems: 'center',
                gap: 1,
            ...
            >
              <ArrowLeft size={8} strokeWidth={3} />
              {step.inputs.length}
            </span>
          )}
        </div>
